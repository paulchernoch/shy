...
fn serialize_references<S>(references_rrro : &Rc<RefCell<Option<References>>>, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
    let references_ro = references_rrro.as_ref().borrow();
    match *references_ro {
        Some(ref refs) => {
...
fn deserialize_references<'de, D>(_deserializer : D) -> Result<Rc<RefCell<Option<References>>>, D::Error> where D: Deserializer<'de> {
